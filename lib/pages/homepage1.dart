...
class _Homepage1State extends State<Homepage1> {
  ...
  Future<void> _loadData() async {
    ...
    await provider.getProducts(); // Load master products first
  ...
  void _groupProductsByCategory() {
    ...
    // Create a map of masterProductId to categoryId
    ...
    print("📋 Master Products mapping: ${productToCategoryMap.length} products mapped to categories");

    // Group GSM products by category using the masterProductId
    for (var gsmProduct in provider.gsmproducts) {
      String? masterProductId = gsmProduct.masterProductId?.id;
      
      if (masterProductId != null && productToCategoryMap.containsKey(masterProductId)) {
        ...
        print("✅ Mapped GSM product '${gsmProduct.name}' to category ID: $categoryId");
      } else {
        print("⚠️ GSM product '${gsmProduct.name}' has no matching category (masterProductId: $masterProductId)");
      }
    }

    print("✅ Grouped ${provider.gsmproducts.length} GSM products into ${categoryProducts.length} categories");
    
    // Print category counts
    categoryProducts.forEach((categoryId, products) {
      var category = masterCategories.firstWhere(
        (cat) => cat.id == categoryId,
        orElse: () => MastercategoryModel(id: '', name: 'Unknown', description: '', categoryImage: ''),
      );
  ...
  void _navigate(MastercategoryModel category) {
    List<Gsmproductmodel> products = categoryProducts[category.id] ?? [];

    print("🔄 Navigating to category: ${category.name} with ${products.length} products");
